...
use OpenAIHydra-v1_backend::model_registry;
use OpenAIHydra-v1_backend::state::{AppState, LogEntry, LogRingBuffer};
use OpenAIHydra-v1_backend::watchdog;
...
async fn build_app(log_buffer: std::sync::Arc<LogRingBuffer>) -> (axum::Router, AppState) {
    ...
    OpenAIHydra-v1_backend::system_monitor::spawn(state.system_monitor.clone());
    ...
    let app = OpenAIHydra-v1_backend::create_router(state.clone())
        ...
        .layer(axum::middleware::from_fn(
            OpenAIHydra-v1_backend::request_id_middleware,
        ))
...
async fn main() -> anyhow::Result<()> {
    ...
    if OpenAIHydra-v1_backend::browser_proxy::is_enabled() {
        let proxy_url = std::env::var("BROWSER_PROXY_URL")
            .unwrap_or_else(|_| "http://localhost:3001".to_string());
        let auto_restart = OpenAIHydra-v1_backend::browser_proxy::proxy_dir().is_some();
        tracing::info!(
