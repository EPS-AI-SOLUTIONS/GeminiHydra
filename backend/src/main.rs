...
use GrokHydra-v1_backend::model_registry;
use GrokHydra-v1_backend::state::{AppState, LogEntry, LogRingBuffer};
use GrokHydra-v1_backend::watchdog;
...
async fn build_app(log_buffer: std::sync::Arc<LogRingBuffer>) -> (axum::Router, AppState) {
    ...
    let app = GrokHydra-v1_backend::create_router(state.clone())
        ...
        .layer(axum::middleware::from_fn(
            GrokHydra-v1_backend::request_id_middleware,
        ))
...
async fn main() -> anyhow::Result<()> {
    ...
    if GrokHydra-v1_backend::browser_proxy::is_enabled() {
        let proxy_url = std::env::var("BROWSER_PROXY_URL")
            .unwrap_or_else(|_| "http://localhost:3001".to_string());
        let auto_restart = GrokHydra-v1_backend::browser_proxy::proxy_dir().is_some();
        tracing::info!(
    ...
    print_banner(port);
    tracing::info!("GrokHydra-v1 v15 backend listening on http://{}", addr);
...
fn print_banner(port: u16) {
    // GrokHydra-v1: bold cyan (36)
    println!();
    println!("  \x1b[1;36m>>>  GrokHydra-v1 v15  <<<\x1b[0m");
