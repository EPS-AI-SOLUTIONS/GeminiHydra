    ...
    for attempt in 1..=2u8 {
        ...
        let resp = match resp {
            ...
            Err(e) => {
                ...
                tracing::error!("browser proxy unavailable: {} — {}", url, e);
                return Err("Browser proxy unavailable".to_string());
            }
        };
        ...
        let resp_text = match resp.text().await {
            ...
            Err(e) => {
                ...
                tracing::error!("browser proxy response read error: {}", e);
                return Err("Browser proxy response read error".to_string());
            }
        };
        ...
        if !status.is_success() {
            ...
            return Err(format!(
                "Browser proxy request failed (HTTP {})",
                status.as_u16()
            ));
        }
...
pub(crate) async fn detailed_health_check(client: &reqwest::Client) -> BrowserProxyStatus {
        ...
        .await
    {
        ...
        Err(e) => {
            tracing::error!("browser proxy health check failed: {}", e);
            BrowserProxyStatus {
                configured: true,
                reachable: false,
                ready: false,
                last_check_epoch: now,
                last_error: Some("Proxy health check failed".to_string()),
                ..Default::default()
            }
        }
    }
