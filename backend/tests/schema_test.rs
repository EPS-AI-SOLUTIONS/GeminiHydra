...
fn openapi_schema_contains_required_fields() {
    ...
    assert!(
        schema.contains("/api/health"),
        "Schema should document /api/health endpoint"
    );
    assert!(
        schema.contains("GeminiHydra"),
        "Schema should contain project name"
    );
}
...
fn openapi_schema_documents_key_endpoints() {
    ...
    assert!(
        schema.contains("/api/models"),
        "Schema should document /api/models"
    );
    assert!(
        schema.contains("/api/sessions"),
        "Schema should document /api/sessions"
    );
}
