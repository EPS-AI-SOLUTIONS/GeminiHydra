...
## Key Files
- `src/features/home/components/WelcomeScreen.tsx` â€” wzorcowy home view (ported to CH/Tissaia)
- `src/shared/hooks/useViewTheme.ts` â€” canonical theme hook (identical in all 3 apps)
- `src/stores/viewStore.ts` â€” sessions, chatHistory, tabs, navigation
- `src/components/atoms/` â€” Badge, Button (shared API across apps)

## Sidebar Logo Gotcha
- Logo + tekst "GeminiHydra" renderowane OBOK SIEBIE (flex row) â€” ogranicza max logo size
- Expanded: `h-36` max (h-48 powoduje overflow tekstu poza sidebar)
- Collapsed sidebar width: 64px â†’ logo max `w-16 h-16`
...
- Key modules: `handlers.rs` (system prompt + tool defs), `state.rs` (AppState + LogRingBuffer), `sessions.rs`, `logs.rs` (4 log endpoints â€” backend/audit/flyio/activity), `tools/` (mod.rs + fs_tools.rs + pdf_tools.rs + zip_tools.rs + image_tools.rs + git_tools.rs + github_tools.rs + vercel_tools.rs + fly_tools.rs), `files.rs`, `analysis.rs` (tree-sitter code analysis), `model_registry.rs` (auto-fetches models from providers at startup, selects best chat/thinking/image model), `browser_proxy.rs` (proxy status + health check + login/logout handlers), `watchdog.rs` (proxy auto-restart + health history), `oauth.rs` (Anthropic OAuth PKCE), `oauth_google.rs` (Google OAuth PKCE + API key), `oauth_github.rs` (GitHub OAuth), `oauth_vercel.rs` (Vercel OAuth), `service_tokens.rs` (Fly.io PAT), `mcp/` (client.rs + server.rs + config.rs), `a2a.rs` (A2A v0.3 protocol)
...
- Image: `pgvector/pgvector:pg16` (NOT standard postgres â€” pgvector extension required for embeddings locally)
...
- Env vars: `DATABASE_URL` (required), `GOOGLE_API_KEY` or `GEMINI_API_KEY`, `ANTHROPIC_API_KEY` (optional), `GOOGLE_OAUTH_CLIENT_ID` + `GOOGLE_OAUTH_CLIENT_SECRET` (optional â€” enables Google OAuth button), `GITHUB_CLIENT_ID` + `GITHUB_CLIENT_SECRET` (optional), `VERCEL_CLIENT_ID` + `VERCEL_CLIENT_SECRET` (optional), `PORT` (default 8081)
...
- DB: Fly Postgres `jaskier-db` â†’ database `geminihydra_v15_backend` (NOT `geminihydra`!)
...
- Current order: 20260214_001 â†’ ... â†’ 20260225_011 â†’ ... â†’ 2026030101_034 â†’ 2026030102_035
...
- Migration 009: pgvector wrapped in DO/EXCEPTION block â€” skips gracefully if extension unavailable
...
- Auto-selects best model per use case using `version_key()` sort (highest version + date wins):
  - **chat**: latest `pro` (excludes lite, image, tts, thinking, computer, robotics, customtools) â†’ `gemini-3.1-pro-preview`
  - **thinking**: latest `flash thinking` (fallback to chat) â†’ `gemini-3.1-pro-preview`
  - **image**: latest model with `image` in ID (excludes robotics, computer) â†’ `gemini-3-pro-image-preview`
- Persists chosen chat model into `gh_settings.default_model` at startup
- No hardcoded model list â€” adapts automatically when Google releases new models
...
## Agent System Prompt
- Defined in `backend/src/handlers.rs` â†’ `build_system_prompt()`
- Contains "CRITICAL: Local Machine Access" section â€” tells agents they run on user's LOCAL machine with FULL filesystem access
- Contains "Tool Selection Rules" section â€” forces Gemini to use `list_directory`/`read_file`/`write_file` instead of `execute_command` for file ops
- Tool definitions in `build_tools()` â€” explicit local filesystem descriptions with Windows path examples
...
- Uses `**Co dalej?**` (NOT `## Co dalej?`) inside `r#""#` raw string â€” `"#` would terminate the raw string
...
- **Web Scraping v2** (tools.rs inline, `web_` prefixed): `fetch_webpage` (SSRF protection, enhanced HTMLâ†’markdown, metadata/OpenGraph/JSON-LD, link categorization, retry+backoff, JSON output), `crawl_website` (robots.txt, sitemap, concurrent JoinSet, SHA-256 dedup, path prefix filter, exclude patterns)
...
- **A2A** (a2a.rs): `call_agent` â€” inter-agent delegation via A2A v0.3 protocol
...
- **Fix 1 â€” Frontend** (`src/shared/hooks/useWebSocketChat.ts`): Heartbeat paused during streaming via `isStreamingRef`, reset on ANY incoming WS message, resumed after `complete`/`error`
- **Fix 2 â€” Backend** (`handlers.rs` handle_ws): Changed from sequential while loop to `tokio::select!` with explicit WebSocket Ping/Pong frame handling concurrent with message processing
- **Fix 3 â€” Backend** (`handlers.rs` build_system_prompt): Added "Tool Selection Rules" â€” forces `list_directory`/`read_file`/`write_file` over `execute_command` for file ops, declares Windows environment
- **Gotcha**: Zod `wsServerMessageSchema` only has start/token/plan/complete/error/pong â€” `tool_call`/`tool_result` types silently dropped by safeParse (tool output rendered via Token messages instead)
...
## Prompt History (Jaskier Shared Pattern â€” identical in CH)
- **Hook**: `usePromptHistory.ts` w `src/features/chat/hooks/` â€” eksportuje `{ promptHistory, addPrompt }`
- **Storage**: DB table `gh_prompt_history` (max 200 wpisĂłw, auto-cleanup) + `localStorage` cache (`'prompt-history-cache'`)
...
- **Integracja**: `ChatContainer.tsx` â†’ `usePromptHistory()` â†’ props `promptHistory` do `ChatInput`
