...
export const DEFAULT_AGENT_PROFILES: Record<string, AgentTemperatureProfile> = {
  dijkstra: {
    name: 'dijkstra', role: 'Strategist',
    baseRanges: { code: [0.85, 1.0], fix: [0.80, 0.95], analysis: [0.95, 1.1], creative: [1.2, 1.5], planning: [0.85, 1.05], general: [0.95, 1.1] },
    creativityBias: -0.05, precisionBias: 0.05, preferredTemp: 0.95, performanceHistory: []
  },
  ciri: {
    name: 'ciri', role: 'Scout',
    baseRanges: { code: [0.95, 1.15], fix: [0.90, 1.1], analysis: [1.0, 1.2], creative: [1.4, 1.7], planning: [1.0, 1.2], general: [1.1, 1.4] },
    creativityBias: 0.10, precisionBias: -0.05, preferredTemp: 1.15, performanceHistory: []
  },
  yennefer: {
    name: 'yennefer', role: 'Architect',
    baseRanges: { code: [0.80, 0.95], fix: [0.80, 0.95], analysis: [0.95, 1.1], creative: [1.2, 1.5], planning: [0.85, 1.05], general: [0.95, 1.15] },
    creativityBias: 0.05, precisionBias: 0.10, preferredTemp: 0.90, performanceHistory: []
  },
  geralt: {
    name: 'geralt', role: 'Security',
    baseRanges: { code: [0.75, 0.90], fix: [0.75, 0.90], analysis: [0.85, 1.0], creative: [1.0, 1.2], planning: [0.80, 0.95], general: [0.85, 1.0] },
    creativityBias: -0.10, precisionBias: 0.15, preferredTemp: 0.85, performanceHistory: []
  },
  triss: {
    name: 'triss', role: 'QA',
    baseRanges: { code: [0.80, 0.95], fix: [0.80, 0.95], analysis: [0.95, 1.1], creative: [1.2, 1.5], planning: [0.95, 1.1], general: [1.0, 1.2] },
    creativityBias: 0.05, precisionBias: 0.05, preferredTemp: 1.0, performanceHistory: []
  },
  lambert: {
    name: 'lambert', role: 'Debugger',
    baseRanges: { code: [0.78, 0.92], fix: [0.75, 0.90], analysis: [0.85, 1.05], creative: [1.0, 1.2], planning: [0.85, 1.05], general: [0.95, 1.1] },
    creativityBias: -0.05, precisionBias: 0.10, preferredTemp: 0.88, performanceHistory: []
  },
  jaskier: {
    name: 'jaskier', role: 'Bard',
    baseRanges: { code: [1.0, 1.2], fix: [0.95, 1.1], analysis: [1.1, 1.3], creative: [1.5, 1.9], planning: [1.1, 1.3], general: [1.3, 1.6] },
    creativityBias: 0.15, precisionBias: -0.10, preferredTemp: 1.45, performanceHistory: []
  },
  regis: {
    name: 'regis', role: 'Researcher',
    baseRanges: { code: [0.85, 1.0], fix: [0.85, 1.0], analysis: [1.0, 1.2], creative: [1.3, 1.55], planning: [0.95, 1.1], general: [1.1, 1.3] },
    creativityBias: 0.05, precisionBias: 0.05, preferredTemp: 1.1, performanceHistory: []
  },
  vesemir: {
    name: 'vesemir', role: 'Mentor',
    baseRanges: { code: [0.85, 1.0], fix: [0.85, 1.0], analysis: [1.0, 1.2], creative: [1.2, 1.5], planning: [0.95, 1.1], general: [1.0, 1.2] },
    creativityBias: 0.0, precisionBias: 0.05, preferredTemp: 1.05, performanceHistory: []
  },
  eskel: {
    name: 'eskel', role: 'DevOps',
    baseRanges: { code: [0.80, 0.95], fix: [0.80, 0.95], analysis: [0.95, 1.1], creative: [1.1, 1.3], planning: [0.85, 1.05], general: [0.95, 1.1] },
    creativityBias: -0.05, precisionBias: 0.10, preferredTemp: 0.95, performanceHistory: []
  },
  zoltan: {
    name: 'zoltan', role: 'Data',
    baseRanges: { code: [0.80, 0.95], fix: [0.80, 0.95], analysis: [0.95, 1.1], creative: [1.1, 1.35], planning: [0.95, 1.1], general: [1.0, 1.2] },
    creativityBias: 0.0, precisionBias: 0.10, preferredTemp: 0.95, performanceHistory: []
  },
  philippa: {
    name: 'philippa', role: 'API',
    baseRanges: { code: [0.80, 0.95], fix: [0.80, 0.95], analysis: [0.95, 1.1], creative: [1.1, 1.35], planning: [0.85, 1.05], general: [0.95, 1.1] },
    creativityBias: 0.0, precisionBias: 0.10, preferredTemp: 0.90, performanceHistory: []
  }
};
...
export class TemperatureController {
  ...
  learnFromResult(
    agentName: string, temperature: number, taskType: TaskType,
    qualityScore: number, responseTime: number = 0, wasSuccessful: boolean = true
  ): void {
    if (!this.config.enableLearning) return;

    const record: TemperaturePerformanceRecord = {
      timestamp: Date.now(), temperature, taskType,
      qualityScore: Math.max(0, Math.min(1, qualityScore)),
      responseTime, wasSuccessful
    };
  ...
  getAgentStats(agentName: string): {
    preferredTemp: number; avgQuality: number; bestTaskType: TaskType | null; totalSamples: number;
  } {
    const profile = this.config.agentProfiles[agentName];
    if (!profile || profile.performanceHistory.length === 0) {
      return { preferredTemp: profile?.preferredTemp || 0.3, avgQuality: 0, bestTaskType: null, totalSamples: 0 };
    }
    ...
    for (const [taskType, scores] of Object.entries(taskTypeScores)) {
      ...
      if (avg > bestAvg) { bestAvg = avg; bestTaskType = taskType as TaskType; }
    }

    return { preferredTemp: profile.preferredTemp, avgQuality: this.round(avgQuality), bestTaskType, totalSamples: history.length };
  }

  private calculateOptimalFromHistory(history: TemperaturePerformanceRecord[], taskType: TaskType): number {
    const relevant = history.filter(r => r.taskType === taskType && r.wasSuccessful && r.qualityScore > 0.5);
  ...
  private updatePreferredTemperature(profile: AgentTemperatureProfile): void {
    ...
    profile.preferredTemp = profile.preferredTemp * (1 - this.config.learningRate) + newPreferred * this.config.learningRate;
  ...
  private createDefaultProfile(agentName: string): AgentTemperatureProfile {
    return {
      name: agentName, role: 'Unknown',
      baseRanges: { code: [0.15, 0.25], fix: [0.10, 0.20], analysis: [0.25, 0.40], creative: [0.40, 0.60], planning: [0.20, 0.35], general: [0.25, 0.45] },
      creativityBias: 0, precisionBias: 0, preferredTemp: 0.3, performanceHistory: []
    };
  }
  ...
  exportLearningState(): {
    ...
    for (const [name, profile] of Object.entries(this.config.agentProfiles)) {
      profiles[name] = { name: profile.name, preferredTemp: profile.preferredTemp, performanceHistory: profile.performanceHistory };
    }
...
export function initializeTemperatureController(config?: Partial<AdaptiveTemperatureConfig>): TemperatureController {
...
export function detectTaskType(taskLower: string): TaskType {
  const codeKeywords = ['implementuj', 'implement', 'kod', 'code', 'funkcj', 'function', 'class', 'klasa', 'metod', 'method', 'zapis', 'write', 'stwórz', 'create', 'dodaj', 'add', 'refactor', 'typescript', 'javascript', 'python', 'rust', 'programuj', 'compile', 'build'];
  const fixKeywords = ['napraw', 'fix', 'bug', 'błąd', 'error', 'debug', 'issue', 'problem', 'popraw', 'correct', 'repair', 'resolve', 'hotfix'];
  const analysisKeywords = ['analiz', 'analy', 'sprawdź', 'check', 'review', 'przegląd', 'evaluate', 'assess', 'audit', 'inspect', 'examine', 'verify', 'test', 'validate', 'compare', 'porównaj'];
  const creativeKeywords = ['propozycj', 'propos', 'sugest', 'suggest', 'pomysł', 'idea', 'creative', 'kreatywn', 'innowac', 'innovat', 'alternatyw', 'alternative', 'brainstorm', 'design', 'projekt', 'koncept', 'concept', 'vision', 'wizja', 'możliwoś', 'possibil'];
  const planningKeywords = ['plan', 'strateg', 'roadmap', 'harmonogram', 'schedule', 'organize', 'struktur', 'architektur', 'architect', 'blueprint', 'diagram', 'workflow', 'process', 'procedur', 'krok', 'step'];
...
export function getAdaptiveTemperature(
  task: string, taskType: TaskType, agentName?: string, context?: Partial<TemperatureContext>
): number {
  ...
  if (agentName) {
    ...
    if (context) {
      const fullContext: TemperatureContext = {
        agentName, taskType: detectedType, task,
...
function getBasicAdaptiveTemperature(task: string, taskType: TaskType): number {
  const temperatureRanges: Record<TaskType, [number, number]> = {
    code: [0.1, 0.2], fix: [0.1, 0.2], analysis: [0.3, 0.4],
    creative: [0.5, 0.7], planning: [0.2, 0.3], general: [0.3, 0.5]
  };
...
export function getEnhancedAdaptiveTemperature(
  ...
  options: {
    taskType?: TaskType; generationProgress?: number; currentStep?: number;
    totalSteps?: number; previousResults?: Array<{ temperature: number; quality: number; wasSuccessful: boolean }>;
    confidenceLevel?: number; retryCount?: number;
  } = {}
  ...
  if (adjusted !== temperature) {
    if (context.generationProgress > 0) adjustments.push(`Annealing (${Math.round(context.generationProgress * 100)}% progress)`);
    if (context.confidenceLevel < 0.5) adjustments.push(`Uncertainty boost (confidence: ${context.confidenceLevel})`);
    if (context.retryCount > 0) adjustments.push(`Retry adjustment (attempt ${context.retryCount + 1})`);
    if (context.previousResults.length > 0) adjustments.push(`Context awareness (${context.previousResults.length} previous results)`);
